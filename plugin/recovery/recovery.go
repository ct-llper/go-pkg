	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				if brokenPipe {
					logx.WithContext(GetContext(c)).Error("%s\n%s%s", err, headersToStr, reset)
					logx.WithContext(GetContext(c)).Info("%s\n%s%s", err, headersToStr, reset)
				} else if gin.IsDebugging() {
					logx.WithContext(GetContext(c)).Error("[Recovery] %s panic recovered:\n%s\n%s\n%s%s",
						timeFormat(time.Now()), headersToStr, err, string(stack), reset)
					logx.WithContext(GetContext(c)).Info("[Recovery] %s panic recovered:\n%s\n%s\n%s%s",
						timeFormat(time.Now()), headersToStr, err, string(stack), reset)
				} else {
					logx.WithContext(GetContext(c)).Error("[Recovery] %s panic recovered:\n%s\n%s%s",
						timeFormat(time.Now()), err, string(stack), reset)
					logx.WithContext(GetContext(c)).Info("[Recovery] %s panic recovered:\n%s\n%s%s",
						timeFormat(time.Now()), err, string(stack), reset)
				}
				if brokenPipe {
...
func stack(skip int) []byte {
	...
	for i := skip; ; i++ { // Skip the expected number of frames
		...
		logx.WithContext(context.TODO()).Error("\t%s: %s\n", function(pc), source(lines, line))
		logx.WithContext(context.TODO()).Info("\t%s: %s\n", function(pc), source(lines, line))
		fmt.Fprintf(buf, "\t%s: %s\n", function(pc), source(lines, line))
	}
	return buf.Bytes()
}
